...
//		for (int i = 0; i < W; ++i) {
//			for (int j = 0; j < H; ++j) {
//				totalRectArea += mask[Fn::fi2(i, j)] > 0 ? 1 : 0;
//			}
//		}
//
//		float nextArea = 0;
...
//	for (i = 0; i < W; i++)
//		sub[Fn::fi2(i, 0)] = mask[Fn::fi2(i, 0)];
//
//	/* Set first row of S[][]*/
//	for (j = 0; j < H; j++)
//		sub[Fn::fi2(0, j)] = mask[Fn::fi2(0, j)];
//
//	/* Construct other entries of S[][]*/
//	for (i = 1; i < W; i++) {
//		for (j = 1; j < H; j++) {
//			if (mask[Fn::fi2(i, j)] == 1)
//				sub[Fn::fi2(i, j)] = min(
//					sub[Fn::fi2(i, j - 1)], 
//					min(sub[Fn::fi2(i - 1, j)], 
//						sub[Fn::fi2(i - 1, j - 1)])) + 1;
//			else
//				sub[Fn::fi2(i, j)] = 0;
//		}
//	}
...
//		in S[][] */
//	max_of_s = sub[Fn::fi2(0, 0)]; max_i = 0; max_j = 0;
//	for (i = 0; i < W; i++) {
//		for (j = 0; j < H; j++) {
